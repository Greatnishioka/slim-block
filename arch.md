...
src/
  core/                   ← React も DOM も知らない
    ...
    rng.ts                決定論的な擬似乱数（mulberry32。seed固定で再現可能）
    solver.ts              「本当に全回収できるか」をDFSで実探索するソルバー
    generate.ts            障害物・スライム・回収口をランダム生成し、solverで解けると
                           確認できた盤面だけを返す（§9.1）
  game/
...
### 9.1 手作りステージ以外に、ソルバー付き自動生成も追加した

当初は仕様 §18 どおり手作りASCIIステージのみを想定していたが、開発中に「詰まない盤面を自動生成してほしい」という要求が生まれたため、`core/generate.ts` を追加した。設計は以下の3点で構成される。

1. **`core/rng.ts`**: mulberry32による決定論的な擬似乱数。同じseedなら常に同じ盤面になる（core全体の「決定論」の思想を生成にも適用。仕様 §21）。
2. **`core/solver.ts`**: 「この盤面は本当に全回収できるか」を実際に探索して確認するソルバー。手の選択肢は常に4方向だけで、1手ごとにセルは減るか同じ数のまま再配置されるだけ（増えることはない）ため、状態空間は見た目ほど大きくならない。**DFS（スタック）＋「その手でセルがより多く減る方向を優先する」ヒューリスティック**で探索する。全滅させるパズルなので貪欲に近い経路を先に試す方が速く、実測でBFSの数秒〜十数秒かかるケースが数百ms程度まで縮んだ。
3. **`core/generate.ts`**: 障害物・スライム・回収口をランダム配置し、`solve()` で実際にクリア手順が見つかった盤面だけを返す。見つからなければ盤面を作り直す（既定80回まで）。**「詰む盤面」が生成されることはない**——ソルバーが見つけた手順そのものが解ける証拠になる。

生成パラメータ（既定値: `slimeGroupCount=10`, `obstacleAttempts=12`, `maxSolveDepth=24`, `maxSolveStates=4000`）は、実測ベンチマーク（60シード試行）で平均約200ms・最大約1.1秒に収まるよう調整済み。`maxSolveStates` を絞って「解けないなら早く諦めて次のシードを試す」方針にすることで、生成一式の体感速度を確保している。

UI側（`ui/App.tsx`）では、この生成を `useEffect` 内で行い、完了まで「生成中…」を表示する。seedが変わったら React の `key` でコンポーネントごと再マウントし、`useGameSession` の内部状態も含めてゼロから作り直す（refの初期値は2回目以降のレンダーでは効かないため、状態リセットには `key` を使うのが定石）。

以下の手作りステージの節（ASCIIレイアウト）は、キャンペーン用ステージなど「意図を込めた配置」が必要な場合に引き続き使う。自動生成は「毎回違う盤面で気軽に遊びたい」用途を担う、並存する2本目の供給経路という位置づけ。
...
| 4 | Drain の通行可否 | 通行可能な床タイルとして扱う。回収判定は**毎tickの直後**に行う（当初は「安定後」の予定だったが、プレイして「移動中に触れても回収されない」のが不自然と判断し変更した。塊の一部が回収口へ触れた瞬間、停止を待たずその場で塊全体を回収する）。床でないDrainも「通過点」として機能するようになった |
...
| 10 | **手作りステージ以外に自動生成を追加** | 仕様にも当初のarch.mdにも存在しなかった要求。「詰まない盤面」を保証するため、ランダム生成＋ソルバーによる実探索検証というアプローチを採った（§9.1）。単なる乱数配置ではなく「実際にクリア手順が見つかった盤面だけ採用する」点が肝で、これにより保証の根拠が曖昧な発見的ルール（「2段以上の障害物は避ける」等）に頼らずに済む |

いずれも仕様を否定するものではないが、実装前に合意しておきたい箇所。特に 1・4・7・8・10 は後から変えると影響範囲が広い。
